...
const SuperAdminDashboard = () => {
  ...
  const fetchInitialData = useCallback(async (silent = false) => {
    ...
    try {
      const [schoolsRes, requestsRes] = await Promise.all([
        API.get(`/superadmin/schools?t=${Date.now()}`),
  ...
  const handleSubmit = async (e) => {
    ...
    const loadingToastId = toast.loading('Registering school tenant...');
    try {
      await API.post('/superadmin/create-school', formData);
      toast.success('School and Admin successfully created!', { id: loadingToastId });
      setShowModal(false);
      setFormData({ schoolName: '', adminName: '', adminEmail: '', adminPassword: '' });
      await fetchInitialData();
    } catch (err) {
      toast.error(err.response?.data?.message || 'Creation failed', { id: loadingToastId });
    } finally {
  ...
  const handleUpdateSchool = async (e) => {
    e.preventDefault();
    setLoadingOperation(true);
    const loadingToastId = toast.loading('Updating school...');
    try {
      ...
      toast.success('School updated successfully!', { id: loadingToastId });
      setShowEditModal(false);
      await fetchInitialData();
    } catch (err) {
      toast.error(err.response?.data?.message || 'Update failed', { id: loadingToastId });
    } finally {
      setLoadingOperation(false);
    }
  };
  ...
  const handleUpdateSchoolSubscription = async (school, action) => {
    setLoadingOperation(true);
    try {
      const { data } = await API.put(`/superadmin/schools/${school.id}/subscription`, { action });

      // Use the direct response as the source of truth
      const updatedSchool = data.school;
      setSchools((prev) => prev.map((s) => (
        Number(s.id) === Number(school.id) && updatedSchool
          ? { ...s, ...updatedSchool, updated_at: new Date().toISOString() }
          : s
      )));

      toast.success(`School ${action === 'pause' ? 'paused' : 'resumed'} successfully`);
      // Increase delay slightly to allow cache invalidation to finalize
      setTimeout(() => fetchInitialData(true), 1500);
    } catch (err) {
  ...
  const handleGlobalPauseToggle = async () => {
    ...
    try {
      ...
      setSchools((prev) => prev.map(s => {
        ...
        return {
          ...s,
          subscription_paused: willBePaused,
          subscription_status: willBePaused ? 'paused' : (
            (!s.subscription_expires_at || new Date(s.subscription_expires_at) >= new Date()) ? 'active' : 'expired'
          ),
          updated_at: new Date().toISOString()
        };
      }));

      setTimeout(() => fetchInitialData(true), 2000); 
    } catch (err) {
